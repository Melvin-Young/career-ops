...
const WEB_FIELD = {
  ...
  score: "score", status: "status", pdf: "pdf", report: "report", notes: "notes", url: "url",
};
 ...
 * @returns {{n: string, date: string, company: string, via: string, role: string, score: string, status: string, pdf: string, report: string, notes: string, url: string}[]}
 */
export function parseApplications(md, rootDir) {
  ...
  for (const raw of lines) {
    ...
    if (map) {
      ...
      rows.push({ n, date, company, via: "", role, score, status, pdf, report, notes: rest.join(" | "), url: "" });
    }
  }
  return rows;
}
